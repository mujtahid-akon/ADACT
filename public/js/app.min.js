...
"_")},copyAllShortNames:function(){for(var a=$("#table_short_names").children().get(1).children,b=0;b<a.length;++b)a[b].children[2].children[0].value=a[b].children[1].textContent.replace(/\s/g,"_")}},Project={result:{MAW:"maw",RAW:"raw",submit_btn:null,config:{},project_id:null,btn_msg_pre:null,prepare:function(){this.config={project_name:$("#project_name").val(),aw_type:$("input[name='aw_type'][value='raw']").is(":checked")?this.RAW:this.MAW,kmer:{min:parseInt($("#kmer_min").val()),max:parseInt($("#kmer_max").val())},
inversion:$("#inversion").is(":checked"),dissimilarity_index:$("#dissimilarity_index").val(),sequence_type:$("input[name='seq_type'][value='protein']").is(":checked")?InputAnalyzer.PROTEIN:InputAnalyzer.NUCLEOTIDE,data:[],type:InputMethod.getCurrent()===InputMethod.ACCN_GIN?InputMethod.ACCN_GIN:InputMethod.FILE};return InputMethod.getCurrent()!==InputMethod.ACCN_GIN&&(this.config.file_id=InputAnalyzer.file_id,null===this.config.file_id)?!1:this.config.project_name&&0<=this.config.kmer.min<=this.config.kmer.max&&
this.config.dissimilarity_index},verify:function(){if(this.prepare()){InputAnalyzer.addShortNames();if(!InputAnalyzer.done)return alert(Messages.Project.UNFILLED_FIELDS),!1;this.config.data=InputAnalyzer.results}else return alert(Messages.Project.UNFILLED_FIELDS),!1;return!0},send:function(){if(!this.verify())return!1;this.submit_btn=$("#p_btn");var a=this;$.ajax({method:"post",url:"./projects/new",data:{config:JSON.stringify(this.config)},cache:!1,dataType:"json",beforeSend:function(){var b=a.submit_btn;
...
a.addClass("btn-primary");a.attr("onclick","Project.result.send()");a.html(this.btn_msg_pre)}},process:{status:function(a,b){$.ajax({method:"post",url:"./projects/get_status",data:{project_id:b},cache:!1,dataType:"json",beforeSend:function(){},success:function(b){a.html(b.status);switch(b.status_code){case 0:window.location.reload();break;case 1:alert(Messages.Project.Status.FAILURE_ALERT),window.location.assign("./projects")}},error:function(){a.html(Messages.CONNECTION_PROBLEM)}})},cancel:function(a,
b){$.ajax({method:"post",url:"./projects/cancel_process",data:{project_id:a},cache:!1,dataType:"json",beforeSend:function(){return confirm(Messages.Project.Cancel.CANCEL_MESSAGE(b))},success:function(a){switch(a.status){case 0:alert(Messages.Project.Cancel.SUCCESS_MESSAGE);window.location.assign("./projects");break;default:alert(Messages.Project.Cancel.FAILURE_MESSAGE)}},error:function(){alert(Messages.CONNECTION_PROBLEM)}})}},delete:function(a,b,c){$.ajax({method:"post",url:"./projects/"+a+"/delete",
cache:!1,dataType:"json",beforeSend:function(){return confirm(Messages.Project.Delete.DELETE_MESSAGE(b))},success:function(b){switch(b.status){case 0:c&&window.location.assign("./projects");$("#p_"+a).remove();break;default:alert(Messages.Project.Delete.FAILURE_MESSAGE)}},error:function(){alert(Messages.CONNECTION_PROBLEM)}})},edit:{MAW:"maw",RAW:"raw",info:{},submit_btn:null,collect:function(){this.info={aw_type:$("input[name='aw_type'][value='raw']").is(":checked")?this.RAW:this.MAW,kmer:{min:parseInt($("#kmer_min").val()),
max:parseInt($("#kmer_max").val())},inversion:$("#inversion").is(":checked"),dissimilarity_index:$("#dissimilarity_index").val()}},send:function(a){this.collect();this.submit_btn=$("#submit_btn");var b=this;$.ajax({method:"post",url:"./projects/"+a+"/edit",data:{config:JSON.stringify(this.info)},cache:!1,dataType:"json",beforeSend:function(){var a=b.submit_btn;a.removeClass("btn-primary");a.addClass("btn-default disabled");a.attr("onclick",null);a.html('<i class="fa fa-spinner fa-pulse" aria-hidden="true"></i> '+

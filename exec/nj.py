...
ps = Graph(format='jpg', filename='NeighbourTree', node_attr={'shape': 'plaintext'})
...
def lowest_cell(table):
    ...
    for i in range(len(table)):
        for j in range(len(table)):
            if i == j:
                ...
                indegree[i] = indegree[i] + table[i][j]
            elif i < j:
                indegree[i] = indegree[i] + table[j][i]
    ...
    for i in range(len(table)):
        for j in range(len(table[i])):
            min_value = ( len(table) - 2 ) * table[i][j] - indegree[i] - indegree[j]
